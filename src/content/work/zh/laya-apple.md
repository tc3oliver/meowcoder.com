...
<a href="https://github.com/tc3oliver/laya-apple" target="_blank" rel="noopener noreferrer">laya-apple</a> 在一台 Mac 上讓 MLX GPU 與 Neural Engine 同時提供服務：長請求交給 GPU，短請求透過 Core ML 交給 Apple Neural Engine。

- **GPU 結果回傳 P50：7.67 → 0.14 ms**，來自針對 GIL 的因果介入實驗。
- **正式版驗證 4.28–8.60 → 0.035–0.043 ms**：laya-apple 1.5 對比 1.4 路徑，吞吐量為 1.4 的 1.038–1.042 倍。
- **12 個慢 episode 全數復原**，最慢 414 ms，預先登錄的上限是 1.0 s。
- **154 個正式驗證 episode**：輸出不一致、路由失敗、請求遺失、當機，全部為 0。

上游：<a href="https://github.com/apple/coremltools/pull/2876" target="_blank" rel="noopener noreferrer"><code>apple/coremltools#2876</code></a>，在原生 `MLModel.predict()` 執行期間釋放 GIL。目前開啟中，尚未合併。
...
可能的原因有四個：GPU、Neural Engine、Python，或是外層的 runtime。每一個都對應不同的修法，不先量測就無從判斷。下文的 episode 指的是 GPU 與 Neural Engine 同時提供服務的一段期間。

## 找到 GIL

最先排除的是 GPU。16 條 GPU stream 裡有 15 條，MLX `mx.eval` 的時間最多只變成原本的 1.04 倍，GPU 運算幾乎沒有變慢。
...
GIL 被長時間持有不是 laya-apple 獨有的問題。任何 Python process 只要在一條 thread 上同步呼叫 Core ML 推論，而其他 thread 也需要直譯器，都可能被同樣拖慢。只在自己的 runtime 裡繞過去，幫不了其他 coremltools 使用者，所以修正提交給 framework 本身：<a href="https://github.com/apple/coremltools/pull/2876" target="_blank" rel="noopener noreferrer"><code>apple/coremltools#2876</code></a>。

這個修改只在原生 `predictionFromFeatures:` 呼叫前後釋放 GIL，對呼叫端來說 `predict()` 仍然是同步的，並附上多執行緒的回歸測試。它依賴另一個修正（apple/coremltools#2827 或 #2829），處理 NumPy 輸入在沒有持有 GIL 時被釋放的問題。這個 PR 尚未合併；在它合併並正式發布之前，coremltools 仍會在整個原生呼叫期間持有 GIL，所以 1.5 的設計不依賴它。
...
但沒有一種能在實際產品的請求組合下確保安全。Process 隔離在兩個模型上都沒過 gate：laya 的短請求 P99 增加 17.5%（上限 5%），typed-decisions 增加 73.3%。釋放 GIL 的 thread 在三個模型上全部失敗，成本轉嫁到 Neural Engine 的短請求。Prebound binding 在只測異質時段的協定下有兩個模型通過，但先前失敗的釋放 GIL thread 在同一協定下也通過了。換成完整協定，prebound binding 在 n = 12 時因無望達標提前停止，短請求 P99 是 1.4 路徑的 1.409 倍。Core ML 非同步 API 在篩選實驗中仍會變慢（見下節），它保留為候選的快速路徑，但不算解法。

這些結果都照原樣保留。Prebound 的 PASS 與 FAIL 各自只在產生它的協定下成立，事後也沒有改寫。
...
完整協定揭露了第二種狀態。以短請求 P99 來看，異質時段分成兩群：正常的落在 10.4–12.3 ms，慢的落在 13.6–21.2 ms。慢的時候，GPU thread 每次 forward 的 CPU 時間從 1.8 ms 升到 6.1 ms，原生 Core ML `predict` 卻沒變（慢 9.67 ms，正常 9.62 ms）。變慢的是 host，不是 Neural Engine。

Core ML 非同步 API 讓 GPU 回傳維持在 0.035 ms，吞吐量也不低於 1.4 路徑，但三個時間窗裡還是有兩個變慢。對這些時間窗做事後分析，慢狀態呈現出一種時間結構：它是每段重疊開始時的一個 episode，持續 0.8–3.4 s。這些 episode 以外，非同步路徑反而比 1.4 快，P99 是 10.4 ms 對 11.9 ms。

逐 thread 的 CPU 計數器顯示，慢狀態和 thread 跑在哪一種核心上有關。在這段過渡期，Neural Engine dispatcher、短請求 client 和 Core ML callback 這幾條 thread 跑在效率核心（E-core）上的比例是 0.91–1.00，穩定狀態下是 0.00；等它們回到效能核心（P-core），過渡期就結束。這也不只發生在某一條 thread 或某個 process：在取樣的六次轉換裡，Neural Engine 這條鏈、父 process 的其他 thread，以及 GPU worker process 全部一起落在 E-core 上（同樣是事後分析）。

之後試了兩種排程介入：dependency QoS override，以及把 dispatcher 設成 USER_INITIATED QoS。兩者都確實生效（QoS 讀回來是 0x19），但沒有任何一次把 Neural Engine dispatcher 從 E-core 移走。QoS 這條路就此停止。

這是很強的相關，不是已證實的因果。這裡沒有任何研究能說明 macOS 為什麼把這些 thread 排到 E-core，本文也不做推測。

## 從預防轉向復原

如果慢狀態只出現在重疊剛開始的時候，就可以用一段保護期躲過去：每個 episode 的前幾個請求先走 1.4 路徑，之後再切換。保護期設為 64 個請求（H64）時，篩選階段的 10 次轉換、確認階段的 16 次轉換，切換時全都沒有落在 E-core。但確認階段仍因離群值檢查失敗（+2.43 ms，上限 +2.0 ms），這個 FAIL 維持不變。
...
Runtime 本身就有可用的訊號。`RequestTrace` 原本是為了看清請求時間花在哪裡而加上的量測，其中的 `prepare_ms` 記錄 Neural Engine 請求被路由之前花在 host 上的時間。超過 0.3 ms 算一次 host 慢；斷路器 C3 在連續三次 host 慢時觸發，讓該 episode 剩下的部分退回 1.4 路徑。
...
**原因**：沒有任何介入能預防慢狀態，觸發原因也還沒找到。建立在未證實原因上的策略，最後只對剛好量到的那幾次有效。
...
驗證期間沒有自然發生慢狀態，所以斷路器一次也沒觸發，正式版的觸發路徑還沒在正式環境中真的走過。復原的證據來自上面的對照實驗，正式版斷路器的邏輯則由單元測試與研究版比對。
...
<span class="state-flow__detail">躲過開頭慢狀態的保護期，在實際 runtime 中失效。</span>
</li>
<li class="state-flow__step">
...
<figcaption class="state-flow__caption">過程中有好幾項研究以 FAIL 收場，結果都保留在紀錄中。</figcaption>
</figure>

真正的收穫，是分清楚哪些已經弄懂、哪些還沒有。GIL 是證實的原因，所以修正已提交上游；E-core 的落點只是相關，所以 runtime 刻意不依賴對它的解釋，而是量測看得到的症狀，並限制使用者受影響的時間。

## 證據與限制

- **只有一台機器。** 所有結果都來自同一台 M4 Max、同一個 macOS 版本，不宣稱適用於其他 Apple 晶片或 macOS 版本。
- **Episode 中途開始的慢狀態，復原時間沒有量過。** 對照實驗裡的慢狀態都從重疊開頭就出現。重播只顯示 C3 偵測得到中途開始的慢狀態，也就是推翻保護期的那一種，沒有證明復原時間有上限。
- **E-core 落點的資料只涵蓋 laya 自己的兩個 process。** 沒有證明是全系統的現象。
